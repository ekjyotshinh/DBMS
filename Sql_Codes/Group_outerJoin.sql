...
SELECT lname, NVL(partydesc, 'No Party') FROM candidate_shinh c, party_shinh p WHERE c.partyid = p.partyid(+);

SELECT NVL(lname, 'No One') AS Individuals, NVL(partydesc, 'No Description') AS Description FROM candidate_shinh c, party_shinh p WHERE c.partyid(+) = p.partyid;