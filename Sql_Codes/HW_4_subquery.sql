...
WHERE ssn IN (SELECT ssn 
              ...
              WHERE class_code = (SELECT class_code 
                                  FROM shinh_class 
...
WHERE class_code NOT IN(SELECT class_code 
                        FROM shinh_class 
                        WHERE class_description IS null);


SELECT fname, lname
FROM shinh_student 
WHERE fname NOT IN ('John', 'Jack', 'Bob') 
AND phone IS NULL 
...
SELECT ssn, fname, lname, (MONTHS_BETWEEN(SYSDATE,dob)/12)/2 AS age_half 
FROM shinh_student 
WHERE fname LIKE 'J%' 
AND MONTHS_BETWEEN(SYSDATE,dob)/12 > 25 
AND ssn IN (SELECT ssn 
            FROM shinh_student_class 
            WHERE class_code IN (SELECT class_code 
                                 FROM shinh_class 
                                 WHERE LOWER(class_description) LIKE '%intro%')) 
ORDER BY age_half DESC;
