...
FROM shinh_student 
WHERE MONTHS_BETWEEN(SYSDATE,dob)/12 > 15;


SELECT ssn, LOWER(lname) || ' ,' || UPPER(fname) AS Full_Name 
FROM shinh_student;


SELECT fname, lname, dob, salary 
FROM shinh_student 
WHERE LOWER(lname) LIKE '%h%' OR LOWER(lname) LIKE '%a%';


SELECT fname, lname, NVL(TO_CHAR(dob),'Not yet born'),salary 
FROM shinh_student 
WHERE (MONTHS_BETWEEN(SYSDATE,dob)/12 BETWEEN 15 AND 25) 
    AND (UPPER(fname) LIKE 'ABR%');


SELECT fname, lname, dob, salary,
    DECODE(SIGN(salary - 20000), -1,'Poor', 'Rich') AS Rich_OR_Poor 
FROM shinh_student 
WHERE dob IS NOT NULL;


SELECT (SQRT(MONTHS_BETWEEN(SYSDATE,dob)/12) + 20) / 5 
FROM shinh_student;


SELECT fname, lname, dob, salary 
FROM shinh_student 
WHERE INITCAP(fname) NOT IN ('John', 'Jack', 'Bob');


SELECT fname, lname, dob, salary 
FROM shinh_student 
WHERE fname LIKE '__b' 
    AND salary > 10000 
    AND phone LIKE '527%';


CREATE TABLE shinh_student2 AS 
SELECT fname, lname, salary*2 AS Double_Salary 
FROM shinh_student 
WHERE LOWER(lname) LIKE '%nn%' AND dob IS NULL;
